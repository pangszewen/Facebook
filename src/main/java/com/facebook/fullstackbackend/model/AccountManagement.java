...
public class AccountManagement {
                ...
                switch(choice){
                    ...
                    case 3: int choiceFriend = 1;
                            while(choiceFriend!=0){
                                System.out.println("<" + user.getNoOfFriends() + " friends>");  // Display number of friends
                                System.out.println("-------------------------");
                                System.out.println("0 - Back");
                                ...
                                user = database.getProfile(user.getAccountID());    // Update the user object (in case user accept friend requests)
                            }
                            break;

                    case 4: int choiceGroup = 1;
                            while(choiceGroup!=0){
                                System.out.println("<" + user.getGroups().size() + " groups>");     // Display number of groups
                                System.out.println("-------------------------");
                                System.out.println("0 - Back");
        ...
        try{
            ...
            while(choice!=0){
                ...
                if(isFriend)
                    ...
                    statusRequest = connection.checkRequest(user, u1);  // Get back the status of friend request from user to u1 
                ...
                switch(choice){
                    ...
                    case 3: int choiceFriend = 1;
                            while(choiceFriend!=0){
                                System.out.println("<" + u1.getNoOfFriends() + " friends>");    // Display number of friends
                                System.out.println("-------------------------");
                                System.out.println("0 - Back");
                    ...
                    case 4: int choiceGroup = 1;
                            while(choiceGroup!=0){  
                                System.out.println("<" + u1.getGroups().size() + " groups>");   // Display number of groups
                                System.out.println("-------------------------");
                                System.out.println("0 - Back");
                    ...
                    case 6: if(isAdmin){
                                ...
                                while(choiceAdmin!=0){
                                    ...
                                    builder.isBanned(u1);
                                    System.out.println("0 - Back");
                                    if(!database.verifyBanStatus(u1))
                                        System.out.println("1 - Ban user");
                                    else
                                        System.out.println("1 - Unban user");
                                    ...
                                    System.out.println("*************************");
                                    switch(choiceAdmin){
                                        case 1: if(!database.verifyBanStatus(u1))
                                                    admin.banUser(u1);
                                                else
                                                    admin.unbanUser(u1);
                                                break;
                                        case 2: graph = admin.deleteAccount(u1, graph);
                                                if(!database.verifyUserExist(u1.getAccountID()))
                                                    return;     // If successfully deleted the user, exit viewing the user's page
                                                else
                                                    break;
        ...
        try{
            ...
            while(choice!=0){
                ...
                if(mutual.size()==0){
                    ...
                    System.out.println("<" + mutual.size() + " mutual friends");    // Display number of mutual friends.
                    System.out.println("-------------------------");
                    System.out.println("0 - Back");
                ...
                if(choice>0 && choice<=mutual.size()){  // If choice in range, view the selected user's page.
    ...
    // Method to display recommended users by Facebook to user.
    public void displayRecommendedUsers(){
        try{
            ArrayList<User> recomUser = connection.recommendedUser(user, graph);

            if(recomUser.size()==0){    // If you are friends or requested to be friends with all the users in Facebook.
                int choice = 1;
                while(choice!=0){   
            ...
            for(int i=0; i<recomUser.size(); i++){
                ...
                while(choice<-1 || choice>3){
                    ...
                    // Check if is friends
                    boolean isFriend = graph.hasEdge(graph, user.getUsername(), recomUser.get(i).getUsername());
                    ...
                    if(i<recomUser.size()-1)
                        ...
                        System.out.println("0 - Refresh");  // Reached the end of recommendation list.
                    if(statusRequest)
                        System.out.println("1 - Cancel friend request");    // If you have sent friend request to recommended user.
                    else if(pendingRequest)
                        System.out.println("1 - Confirm request");  // If recommended user has sent friend request to user.
                    else if(isFriend)
                    ...
                    switch(choice){
                        ...
                        case 1: if(statusRequest)
                                    ...
                                    graph = connection.removeFriend(user, recomUser.get(i), graph);
                                else
    ...
    // Method to display all groups of user
    public User displayGroups(User u1){
        try{
            ...
            while(choice!=0){
                ...
                System.out.println("<" + groupsID.size() + " groups>");     // Display number of groups
                System.out.println("-------------------------");
                System.out.println("0 - Back");
                ...
                sc.nextLine();
                System.out.println("*************************");
                if(choice>0 && choice<=groups.size())           // If choice in range, view the selected group page.
                    viewGroupPage(groups.get(choice-1));
                u1 = database.getProfile(u1.getAccountID());    // Update user object (in case user leave group)
            }
            return u1;
        }catch(InputMismatchException e){
            System.out.println("*************************");
    ...
    public void displayMembers(Group group, User user){
        try{
            ...
            while(choice!=0){
                ...
                System.out.println("<" + members.size() + " members>");     // Display number of members
                System.out.println("-------------------------");
                System.out.println("0 - Back");
                ...
                if(choice>0 && choice<=members.size()){
                    ...
                    if(u1.getAccountID().equals(user.getAccountID()))   // If selected user is user himself, view his own page.
                        viewMyPage();
                    else    
                        viewOtherPage(u1);
                }
                group = database.getGroup(group.getGroupID());  // Refresh Group object (in case member of group is deleted from Facebook by admin)
            }
    ...
    // Method to display group invitations.
        ...
        try{
            ...
            while(choice!=0){
                ...
                for(int i=0; i<groupInvitations.size(); i++){
                    ...
                    }else{      // When group was deleted by admin after invitation was sent to user
                        groupManager.deleteInvitation(xArr[1], user);   
                ...
                System.out.println("<" + groupInvitations.size() + " invitations>");    // Display number of invitations
                System.out.println("-------------------------");
                System.out.println("0 - Back");
                for(int i=0; i<groups.size(); i++){
                    System.out.println((i+1) + " - " + groups.get(i).getGroupName());
                ...
                while(choiceInvite==1 || choiceInvite<0 || choiceInvite>3){
                    ...
                    if(groupManager.isMember(group, user))  // If user join group not through invitation, back to list of group invitations.
                        break;
                }
            }
        }catch(InputMismatchException e){
    ...
    // Method to display user's history of viewed posts.
    public void displayHistory(){
        try{
            System.out.println("\tHistory");
            System.out.println("-------------------------");
            int choice = 1;
            if(history.getSize()==0){       // If there is no history
                while(choice!=0){   
            ...
            }else{      // Iterate through the history linked list
